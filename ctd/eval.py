...
def calculate_dists(samples, descriptors, dist_type='l2'):
    ...
    if dist_type == 'l2':
        ...
        dists = np.abs(1 - np.sum(feats1 * feats2, axis=1) /
...
def eval_verification(samples, descriptors, visualize=False, dist_type='l2'):
    ...
    true_positives = [(p, pair) for p, pair in zip(
        predictions_mean, pairs) if p and pair[2]]
    true_negatives = [(p, pair) for p, pair in zip(
        predictions_mean, pairs) if (not p) and (not pair[2])]
    false_positives = [(p, pair) for p, pair in zip(
        predictions_mean, pairs) if p and (not pair[2])]
    false_negatives = [(p, pair) for p, pair in zip(
        predictions_mean, pairs) if (not p) and pair[2]]
...
def calculate_query_results(queries, data, query_labels, data_labels, k, dist_type):
    ...
    model = NearestNeighbors(n_neighbors=k,
                             n_jobs=-1,
                             metric='minkowski' if dist_type == 'l2' else dist_type)
    model.fit(data)
...
def eval_retrieval(samples, descriptors, k, dist_type):
    ...
    old_results = calculate_query_results(
        X_old, X_new, y_old, y_new, k, dist_type)
    old_results = np.stack(old_results).astype(np.float)
    ...
    new_results = calculate_query_results(
        X_new, X_old, y_new, y_old, k, dist_type)
    new_results = np.stack(new_results).astype(np.float)
...
def main(args):
    ...
    if args.task == 'verification':
        eval_verification(samples, descriptors,
                          visualize=True, dist_type=args.dist_type)
